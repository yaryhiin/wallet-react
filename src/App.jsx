...
function App() {
    ...
    async function loadData() {
      ...
      try {
        ...
        if (categoriesData.length === 0) {
          const newExpenseCategories = defaultExpenseCategories.map((c) => ({
            user_id: session.user.id,
            type: "expense",
            name: c,
          }));
          const newIncomeCategories = defaultIncomeCategories.map((c) => ({
            user_id: session.user.id,
            type: "income",
            name: c,
          }));
  ...
  async function addTransaction(transaction, targetAccount) {
    const newTransaction = await createData("transactions", {
      user_id: session.user.id,
      ...transaction,
    });
    ...
    const changedAccount = updateAccountBalance(
      accounts,
      targetAccount.id,
      transactionAmount,
    );
  ...
  async function changeTransaction(changed) {
    ...
    setAccounts((prev) =>
      prev.map((a) =>
        a.id === updatedAccountOld.id
          ? { ...updatedAccountOld }
          : a.id === updatedAccountNew.id
            ? { ...updatedAccountNew }
            : a,
      ),
    );
  ...
  async function deleteTransaction(id) {
    ...
    const changedAccount = accounts.find(
      (a) => a.id === deletedTransaction.account_id,
    );
    ...
    const transactionDeleted = await deleteData(
      "transactions",
      id,
      session.user.id,
    );
  ...
  async function transfer(from, to, amount, date, exchangeRate) {
    ...
    const changedAccount1 = updateAccountBalance(
      accounts,
      fromAccount.id,
      -amount,
    );
    const changedAccount2 = updateAccountBalance(
      accounts,
      toAccount.id,
      amountExchanged,
    );
    ...
    const newTransaction1 = await createData("transactions", {
      user_id: session.user.id,
      category: "Transfer",
      amount: amount,
    ...
    const newTransaction2 = await createData("transactions", {
      user_id: session.user.id,
      category: "Transfer",
  ...
  async function addAccount(account) {
    const newAccount = await createData("accounts", {
      user_id: session.user.id,
      ...account,
    });
          ...
          {!session ? (
            ...
            <>
              ...
              >
                ...
                <Route
                  path="/addAccount"
                  element={<AddAccount addAccount={addAccount} back={true} />}
                />
